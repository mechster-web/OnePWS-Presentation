...
export function CustomerPathSelector() {
  ...
  function applyPath() {
    const journey = navigationJourneys.find((item) =>
      (!item.selection?.role || item.selection.role === state.customerPath.role) &&
      (!item.selection?.industry || item.selection.industry === state.customerPath.industry)
    );
    dispatch({ type: "SET_ACTIVE_JOURNEY", journeyId: journey?.id ?? "complete-story", startAtOpeningDestination: true });
  }
